...
const IkonWaktu = ({ waktu }) => {
  ...
  return isPM ? (
    <svg xmlns="http://www.w3.org/2000/svg" height="16px" viewBox="0 -960 960 960" width="16px" fill="#cccccca6">
    ...
    <svg xmlns="http://www.w3.org/2000/svg" height="16px" viewBox="0 -960 960 960" width="16px" fill="#f19e39ad">
...
const WaktuSekarang = () => {
  const dapatkanWaktu = () => new Date().toLocaleTimeString("en-US", { hour12: true });
...
const stageInfo = {
  [STAGE.INIT]:      { icon: "⏳", text: "Kamera aktif — menunggu pemindaian...", color: "#aaa" },
  [STAGE.SCANNING]:  { icon: "🔍", text: "Memindai wajah...", color: "#74c0fc" },
  [STAGE.ANALYZING]: { icon: "🧠", text: "Menganalisis & mencocokkan wajah...", color: "#ffd43b" },
  [STAGE.SUCCESS]:   { icon: "✅", text: "Verifikasi berhasil! Akses diberikan.", color: "#51cf66" },
  [STAGE.REJECTED]:  { icon: "❌", text: "Wajah tidak cocok — Akses DITOLAK.", color: "#ff6b6b" },
  [STAGE.NO_FACE]:   { icon: "👤", text: "Wajah tidak terdeteksi. Posisikan wajah di dalam oval.", color: "#ffa94d" },
  [STAGE.ERROR]:     { icon: "⚠️", text: "Kamera belum siap. Harap tunggu sebentar.", color: "#ffa94d" },
  [STAGE.SAVING]:    { icon: "💾", text: "Menyimpan foto referensi wajah...", color: "#74c0fc" },
  [STAGE.SAVED]:     { icon: "✅", text: "Wajah pemilik berhasil disimpan!", color: "#51cf66" },
};
...
export default function Navbar({ isAuthenticated, setIsAuthenticated }) {
  ...
  // Auto-buka modal jika akses /plant tanpa auth (refresh protection)
  useEffect(() => {
  ...
  const handleMenuClick = (e, _index, item) => {
    if (item.title === "Plant Controls" && !isAuthenticated) {
      e.preventDefault();
      setShowVerifyModal(true);
      setStage(STAGE.INIT);
      setConfidence(null);
  ...
  const captureCroppedBlob = useCallback(async () => {
    ...
    outCanvas.getContext("2d").drawImage(fullCanvas, cropX, cropY, cropW, cropH, 0, 0, 300, 300);

    return new Promise((resolve) => outCanvas.toBlob(resolve, "image/jpeg", 0.92));
  }, [webcamRef]);

  // Smart Auto Verify — dengan tahapan jelas
  const autoVerify = useCallback(async () => {
    ...
    try {
      ...
      // Tampilkan confidence jika ada
      if (result.confidence !== undefined) {
        setConfidence(Math.round(result.confidence * 100));
      }

      if (result.status === "failed" && result.message?.includes("tidak terdeteksi")) {
        setStage(STAGE.NO_FACE);
      } else if (result.status === "success" || result.allowed === true) {
        ...
        // Setelah 1.5 detik tampilkan rejected, kembali ke scanning
        setTimeout(() => {
          if (!isAuthenticated) setStage(STAGE.INIT);
        }, 1500);
      }
    } catch (err) {
      if (err.message === "camera_not_ready") {
  ...
  // Auto scan loop: interval 800ms — lebih responsif
  useEffect(() => {
    if (!showVerifyModal || isAuthenticated) return;

    const initialTimer = setTimeout(() => autoVerify(), 600);
    const intervalId = setInterval(() => {
      if (!isAuthenticated) autoVerify();
    }, 800);
  ...
  return (
    <>
      <nav className={`navbar ${isOpen ? "active" : ""}`}>
        ...
        <p className={`apps-name-mobile ${isOpen ? "" : "active"}`}>[ navigation ]</p>
      ...
      <button onClick={openMenu} className={`menu-opener ${isOpen ? "active" : ""}`}>
        {isOpen ? (
      ...
      {showVerifyModal && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p>
              Pemindaian <b>otomatis</b> setiap saat. Posisikan wajah di dalam oval.
            </p>
            ...
            <div className="webcam-container">
              ...
              />
              {/* Scanner line hanya aktif saat scanning/analyzing */}
              {(stage === STAGE.SCANNING || stage === STAGE.ANALYZING) && (
            ...
            {/* Status Box */}
            <div className="verify-status-box" style={{ borderColor: currentStageInfo.color }}>
              <span className="verify-status-icon">{currentStageInfo.icon}</span>
              <div>
                <p className="verify-status-text" style={{ color: currentStageInfo.color }}>
                  {currentStageInfo.text}
                </p>
                {confidence !== null && (
                  <p className="verify-confidence">
                    Skor Kecocokan: <strong style={{ color: confidence >= 22 ? "#51cf66" : "#ff6b6b" }}>{confidence}%</strong>
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="modal-actions">
              <button
                ...
                disabled={stage === STAGE.SAVING || stage === STAGE.SUCCESS}
                className="btn-verify"
              >
                {stage === STAGE.SAVING ? "Menyimpan..." : "Simpan Muka"}
              </button>
              <button
                onClick={handleCancelModal}
                disabled={stage === STAGE.SUCCESS}
